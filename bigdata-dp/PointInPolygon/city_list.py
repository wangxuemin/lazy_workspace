...
# 方法： 计算该点的水平线与多边形的交点 ,交点为奇数，则在多边形内
# 参数： point,polygon (点，多边形)
def pointInPolygonArray( _point, _polygon ):
...
class BitMap:
    ...
    def isMargin( self, point ):
        return self.isMargin( point[0], point[1] )

    def isMargin( self, key1, key2 ):
        if self.getBitMapValue( key1-1 , key2 ) != self.getBitMapValue( key1, key2 ) and self.getBitMapValue( key1-1 , key2 ) != -1:
            return False
        if self.getBitMapValue( key1 , key2-1 ) != self.getBitMapValue( key1, key2 ) and self.getBitMapValue( key1 , key2-1 ) != -1:
            return False
        if self.getBitMapValue( key1+1 , key2 ) != self.getBitMapValue( key1, key2 ) and self.getBitMapValue( key1+1 , key2 ) != -1:
            return False
        if self.getBitMapValue( key1 , key2+1 ) != self.getBitMapValue( key1, key2 ) and self.getBitMapValue( key1 , key2+1 ) != -1:
...
class CityPolygon:
    ...
    def buildCityGeoBitMap(self, city_wkt ,cityid):
        ...
        for idx_lng in range( int(min_lng*scale), int(max_lng*scale) ):
            for idx_lat in range( int(min_lat*scale), int(max_lat*scale) ):
                ...
                #if pointInPolygon( Point(lng,lat), [Point(pt[0],pt[1]) for pt in city_wkt] ):
                if pointInPolygonArray( [lng,lat], city_wkt ):
                    logging.debug( "setBitMapValue %d %d %d", idx_lng, idx_lat, cityid )
                    self.bitmap_.setBitMapValue( idx_lng, idx_lat, cityid )
    ...
    def getCityByPoint(self,x,y):
        ...
        #if self.bitmap_.isMargin( key1, key2 ):
        #    # pointInPolygonArray
        #    pass
        ## 如果不是margin, 直接返回bitmap标记
        #else:
        cityid = self.bitmap_.getBitMapValue( key1,key2 ) 
        logging.debug( "getCityByPoint cityid: %d", cityid )
        return self.id2city_[ cityid ]

if __name__ == '__main__':
    cityPolygon = CityPolygon( WKT )
