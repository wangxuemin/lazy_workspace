...
{
    const LONGITUTE_MAX = 180;
    const LONGITUTE_MIN = -180;
    const LATITUTE_MAX = 74;
    const LATITUTE_MIN = -74;

    public function __construct($lng, $lat)
    {
        $this->lng = $this->filterLongitute($lng);
        $this->lat = $this->filterLatitute($lat);
    }
    ...
    static public function filterLatitute($lat) 
    {
        $lat = max($lat, self::LATITUTE_MIN);
        $lat = min($lat, self::LATITUTE_MAX);
        return $lat;
    }
    static public function filterLongitute($lng) 
    {
...
var_dump($p1->toMC()->toLL()->toArray(), $p1->toMC()->toArray());
