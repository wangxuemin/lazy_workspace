...
//		ConfigUtil.loadResources();
	...
	public void getTaskDiagnostics(String job) {
		...
		try {
			...
			sb.append(String.format("m %4d %6s", mapnum, progress == 1 ? "100%"
					: String.format("%.2f%%", progress * 100)));
			sb.append("|");
			progress = status.getReduceProgress();
			sb.append(String.format("r %4d %6s", rednum, progress == 1 ? "100%"
					: String.format("%.2f%%", progress * 100)));
	...
	private void printTaskAttempt(TaskReport[] tasks) {
		...
		for (TaskReport task : tasks) {
			...
			sb.append("|"
					+ (progress == 1 ? "100%" : String.format("%.2f%%",
							progress * 100)));
			sb.append("|" + sDateTime);
			sb.append("|" + eDateTime);
			sb.append("|"
					+ String.format(
							"% 8.3fs",
							(task.getFinishTime() - task.getStartTime()) / 1000.0));
