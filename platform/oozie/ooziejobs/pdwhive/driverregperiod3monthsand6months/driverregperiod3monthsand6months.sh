...
#procedure name: p_dm_driverregperiod3monthsand6months
#creator:MARS
#created:
#!/bin/bash
#today date
...
# $fromDate=$this->sixMonths, 
# $toDate=$v_threeMonths
/home/xiaoju/hive-0.10.0/bin/hive -e "use app;
INSERT OVERWRITE TABLE DRIVERREGPERIOD3MONTHSAND6MONTHS PARTITION(YEAR='$V_PARYEAR',MONTH='$V_PARMONTH',DAY='$V_PARDAY')
SELECT 0,
       '$V_YESTERDAY',
	   M.AREA,
	   M.CHANNEL,
...
SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
  ...
  FROM (SELECT AREA, 
		...
		GROUP BY AREA, (CASE WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
							ELSE 2
							END)
		GROUPING SETS ( AREA,
						(CASE WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
...
;"
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/driverregperiod3monthsand6months/$V_PARYEAR/$V_PARMONTH/$V_PARDAY/_SUCCESS	