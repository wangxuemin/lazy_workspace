...
ALTER TABLE WECHAT_SENDITEM_DAY ADD IF NOT EXISTS PARTITION (YEAR = '$V_PARYEAR',MONTH = '$V_PARMONTH',DAY = '$V_PARDAY') LOCATION '/user/xiaoju/data/bi/app/wechat_senditem_day/$V_PARYEAR/$V_PARMONTH/$V_PARDAY';
...
(SELECT CASE WHEN E.AREA is NULL THEN '10000' ELSE E.AREA END AREA,
 ...
 (SELECT DISTINCT AREA,PHONE 
  ...
  AND MONTH='$V_PARMONTH'
  AND DAY='$V_PARDAY'
  AND SUBSTR(CREATE_TIME,1,10) != '$V_YESTERDAY' 
  AND  CNT_ITEMS != LEFT_ITEMS
 ) F 
