...
#procedure name: P_DM_todaycallpassenger
#creator:MARS
#created:
#!/bin/bash
#today date
...
INSERT OVERWRITE TABLE TODAYCALLPASSENGER PARTITION(YEAR='$V_PARYEAR',MONTH='$V_PARMONTH',DAY='$V_PARDAY')
SELECT 0,
       '$V_YESTERDAY',
	    M.AREA,
	    M.CHANNEL,
		(CASE WHEN A.TODAYCALLPASSENGER IS NOT NULL THEN CAST(A.TODAYCALLPASSENGER AS INT) ELSE 0 END) TODAYCALLPASSENGER,
		(CASE WHEN A.TODAYCALLSUCCPASSENGER IS NOT NULL THEN CAST(A.TODAYCALLSUCCPASSENGER AS INT) ELSE 0 END) TODAYCALLSUCCPASSENGER,
		(CASE WHEN A.TODAYCALLSUCCPASSENGER IS NOT NULL AND A.TODAYCALLPASSENGER IS NOT NULL  AND A.TODAYCALLPASSENGER<>0
		        THEN ROUND(A.TODAYCALLSUCCPASSENGER/A.TODAYCALLPASSENGER,3)
		   ELSE 0.000
		   END) TODAYCALLPASSENGERSUCCRATE
...
SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
  ...
  FROM (SELECT AREA,
			  (CASE WHEN CHANNEL=48 OR CHANNEL=106 OR CHANNEL=1105 THEN 4
				  ...
				  ELSE 2 
				END) CHANNEL,
				COUNT(DISTINCT PASSENGERID) TODAYCALLPASSENGER,
				COUNT(DISTINCT (CASE WHEN STATUS>=1 AND STATUS<=4 AND DRIVERID>0 THEN PASSENGERID
					   ELSE 0
					   END))-1  TODAYCALLSUCCPASSENGER
		  FROM PDW.DW_ORDER
...
;"
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/todaycallpassenger/$V_PARYEAR/$V_PARMONTH/$V_PARDAY/_SUCCESS