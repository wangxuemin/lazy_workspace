#function description: driverapprovedtwoweeks
#author:changyan
...
TWO_WEEK_FIRST_DAY=`date --date="${TODAY}-14 day" +%Y-%m-%d`
TWO_WEEK_FIRST_DAY_STR=`date --date="${TODAY}-14 day" +%Y%m%d`
MONTH_FIRST_DAY=`date --date="${TODAY}-30 day" +%Y-%m-%d`
MONTH_FIRST_DAY_STR=`date --date="${TODAY}-30 day" +%Y%m%d`

#insert recorders from pdw.Driver/DriverZipper to app.DriverApprovedTwoWeeks
/home/xiaoju/hive-0.10.0/bin/hive -e  "use app;
    INSERT OVERWRITE TABLE DRIVERAPPROVEDTWOWEEKS PARTITION(YEAR='${LAST_DAY_YEAR}',MONTH='${LAST_DAY_MONTH}',DAY='${LAST_DAY}')
        SELECT 0,
			  '${YESTERDAY}',
			  Z.AREA,
			  Z.CHANNEL,
	    ...
	    FROM PDW.DRIVER_AREA_CHANNEL Z
              ...
              WHERE YEAR='${LAST_DAY_YEAR}'
                    ...
                    AND DAY='${LAST_DAY}'
                    AND CHECKDATE >= '${TWO_WEEK_FIRST_DAY}' 
                    AND CHECKDATE < '${TODAY}'
              GROUP BY AREA,
                          ( 
                            CASE
			...
			GROUPING SETS (AREA,
             ...
             WHERE YEAR='${LAST_DAY_YEAR}' AND MONTH='${LAST_DAY_MONTH}' AND DAY='${LAST_DAY}'
                   AND BEGINDATE = '${YESTERDAY}' AND CHECKDATE >= '${TWO_WEEK_FIRST_DAY}' 
			...
			GROUPING SETS (AREA,
	         ...
	         LEFT OUTER JOIN (
			  SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
				...
				(			 
					 ...
					 FROM PDW.DRIVERZIPPER 
					 WHERE CONCAT(YEAR,MONTH,DAY) >= '${TWO_WEEK_FIRST_DAY_STR}'
						   AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
						   AND BEGINDATE >= '${TWO_WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
						   AND ACTIVEDATE >= '${TWO_WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' 
					 GROUP BY AREA,
								  ( 
									CASE
									WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
			 ...
			 WHERE CONCAT(YEAR,MONTH,DAY) >= '${TWO_WEEK_FIRST_DAY_STR}'
				   AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
                   AND BEGINDATE >= '${TWO_WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
                   AND ACTIVEDATE >= '${TWO_WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' AND ONLINEMINS > 0
             GROUP BY AREA,
                          ( 
                            CASE
                            WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
			...
			GROUPING SETS (AREA,
			 ...
			 WHERE CONCAT(YEAR,MONTH,DAY) >= '${TWO_WEEK_FIRST_DAY_STR}'
				   AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
                   AND BEGINDATE >= '${TWO_WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
                   AND ACTIVEDATE >= '${TWO_WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' AND ABOARDORDERCNT > 0
             GROUP BY AREA,
                          ( 
                            CASE
                            WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
	          ...
	          FROM ( SELECT AREA,
					   ...
					   FROM (SELECT AREA,
							 ...
							 JOIN
								...
								WHERE CONCAT(YEAR,MONTH,DAY) >= '${TWO_WEEK_FIRST_DAY_STR}'
								AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
								AND CREATETIME >= '${TWO_WEEK_FIRST_DAY}' AND CREATETIME < '${TODAY}' AND DCOMPLAINTTYPE > 0
								)TEMP2
							 ON (TEMP1.MSISDN = TEMP2.DPHONE)
						 GROUP BY AREA,CHANNEL
						 GROUPING SETS (AREA,
						                CHANNEL,
										(AREA,CHANNEL),
										())
...
;" 
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/driverapprovedtwoweeks/$LAST_DAY_YEAR/$LAST_DAY_MONTH/$LAST_DAY/_SUCCESS
