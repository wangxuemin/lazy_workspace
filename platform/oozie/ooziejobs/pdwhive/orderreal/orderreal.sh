...
# orderSuccCnt     	                   INT     ,
# orderSuccRate    	                   FLOAT   ,
# orderBroadcastCnt	                   INT     ,
# orderStriveCnt   	                   INT     ,
# orderStriveRate  	                   FLOAT
# )
#$table["orderStriveRate"] = $this->divP($table["orderStriveCnt"], $table["orderBroadcastCnt"]);
#orderCallCnt,orderCnt,orderAnswerCnt,orderSuccCnt,orderSuccRate
/home/xiaoju/hive-0.10.0/bin/hive -e"use app;
INSERT OVERWRITE TABLE ORDERREAL PARTITION(YEAR='$V_PARYEAR',MONTH='$V_PARMONTH',DAY='$V_PARDAY')
SELECT 0,
		'$V_YESTERDAY',
		M.AREA,
		M.CHANNEL,
		...
		(CASE WHEN B.ORDERBROADCASTCNT IS NULL THEN 0 ELSE CAST(B.ORDERBROADCASTCNT AS INT) END),
		(CASE WHEN C.ORDERSTRIVECNT IS NULL THEN 0 ELSE CAST(C.ORDERSTRIVECNT AS INT) END),
		(CASE WHEN C.ORDERSTRIVECNT IS NOT NULL AND B.ORDERBROADCASTCNT IS NOT NULL AND B.ORDERBROADCASTCNT <>0 THEN
		       ROUND(C.ORDERSTRIVECNT/CAST(B.ORDERBROADCASTCNT AS FLOAT),3)
			   ELSE 0.000
			   END)
...
SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
  ...
  FROM (SELECT AREA, 
			  ...
			  SUM(CASE WHEN STATUS > 0 AND STATUS < 4  THEN 1
					 ELSE 0
				   END) ORDERSUCCCNT,
			  ROUND(SUM(CASE WHEN STATUS > 0 AND STATUS < 4  THEN 1
					 ELSE 0
				   END)/CAST(COUNT(*) AS FLOAT),3) ORDERSUCCRATE
		 FROM PDW.DW_ORDER
...
SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
 ...
 FROM  (SELECT AREA, 
			  (CASE WHEN CHANNEL=48 OR CHANNEL=106 OR CHANNEL=1105 THEN 4
					...
					ELSE 2 
					END)  CHANNEL,
			   COUNT(*)  ORDERBROADCASTCNT 
		 FROM PDW.ORDERBROADCAST 
		WHERE YEAR='$V_PARYEAR'
		  AND MONTH='$V_PARMONTH'
		  AND DAY='$V_PARDAY'
		  AND TIME >= '$V_YESTERDAY' 
		  AND TIME < '$V_TODAY' 
		  AND TYPE = 0
                   AND AREA IS NOT NULL 
		GROUP BY AREA, (CASE WHEN CHANNEL=48 OR CHANNEL=106 OR CHANNEL=1105 THEN 4
...
SELECT (CASE WHEN AREA IS NULL THEN 10000 ELSE CAST(AREA AS INT) END) AREA,
  ...
  FROM (SELECT AREA, 
			  (CASE WHEN CHANNEL=48 OR CHANNEL=106 OR CHANNEL=1105 THEN 4
					...
					ELSE 2 
					END) CHANNEL,
			   COUNT(*) ORDERSTRIVECNT 
		FROM PDW.ORDERSTRIVE 
		WHERE YEAR='$V_PARYEAR'
		  AND MONTH='$V_PARMONTH'
		  AND DAY='$V_PARDAY'
		  AND TIME >= '$V_YESTERDAY'   
...
;"
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/orderreal/$V_PARYEAR/$V_PARMONTH/$V_PARDAY/_SUCCESS