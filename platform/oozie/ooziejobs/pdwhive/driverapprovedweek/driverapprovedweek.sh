#function description: driverapprovedweek
#author:changyan
...
#insert recorders from pdw.Driver/DriverZipper to app.DriverApprovedWeek
/home/xiaoju/hive-0.10.0/bin/hive -e  "use app;
    INSERT OVERWRITE TABLE DRIVERAPPROVEDWEEK PARTITION(YEAR='${LAST_DAY_YEAR}',MONTH='${LAST_DAY_MONTH}',DAY='${LAST_DAY}')
        SELECT 0,
			  '${YESTERDAY}',
			  Z.AREA,
			  Z.CHANNEL,
		...
		GROUPING SETS (AREA,
             ...
             WHERE YEAR='${LAST_DAY_YEAR}' AND MONTH='${LAST_DAY_MONTH}' AND DAY='${LAST_DAY}'
                   AND BEGINDATE = '${YESTERDAY}' AND CHECKDATE >= '${WEEK_FIRST_DAY}' 
		...
		GROUPING SETS (AREA,
			 ...
			 WHERE CONCAT(YEAR,MONTH,DAY) >= '${WEEK_FIRST_DAY_STR}'
                   ...
                   AND BEGINDATE >= '${WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
                   AND ACTIVEDATE >= '${WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' 
             GROUP BY AREA,
                          ( 
                            CASE
                            WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
		...
		GROUPING SETS (AREA,
			 ...
			 WHERE CONCAT(YEAR,MONTH,DAY) >= '${WEEK_FIRST_DAY_STR}'
			       AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
                   AND BEGINDATE >= '${WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
                   AND ACTIVEDATE >= '${WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' AND ONLINEMINS > 0
             GROUP BY AREA,
                          ( 
                            CASE
                            WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
		...
		GROUPING SETS (AREA,
			 ...
			 WHERE CONCAT(YEAR,MONTH,DAY) >= '${WEEK_FIRST_DAY_STR}'
			       AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
                   AND BEGINDATE >= '${WEEK_FIRST_DAY}' AND BEGINDATE < '${TODAY}' 
                   AND ACTIVEDATE >= '${WEEK_FIRST_DAY}' AND ACTIVEDATE < '${TODAY}' AND ABOARDORDERCNT > 0
             GROUP BY AREA,
                          ( 
                            CASE
                            WHEN CHANNEL >= 202 AND CHANNEL <= 204 THEN 1 
			       ...
			       (SELECT AREA,
					  ...
					  FROM (SELECT AREA,
						...
						JOIN
							...
							WHERE CONCAT(YEAR,MONTH,DAY) >= '${WEEK_FIRST_DAY_STR}'
							AND CONCAT(YEAR,MONTH,DAY) < '${TODAY_STR}'
							AND CREATETIME >= '${WEEK_FIRST_DAY}' AND CREATETIME < '${TODAY}' AND DCOMPLAINTTYPE > 0
							)TEMP2
...
;" 
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/driverapprovedweek/$LAST_DAY_YEAR/$LAST_DAY_MONTH/$LAST_DAY/_SUCCESS
