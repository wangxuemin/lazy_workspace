...
#procedure name: P_DM_drivernatureregthreeweek
#creator:MARS
#created:
#!/bin/bash
#today date
...
INSERT OVERWRITE TABLE DRIVERNATUREREGTHREEWEEK PARTITION(YEAR='$V_PARYEAR',MONTH='$V_PARMONTH',DAY='$V_PARDAY')
SELECT 0,
	   '$V_YESTERDAY',
	   M.AREA,
	   M.CHANNEL,
	   ...
	   (CASE WHEN C.WEEKONLINEDRIVERCNT IS NOT NULL AND A.DRIVERCNT IS NOT NULL AND A.DRIVERCNT<>0
                THEN ROUND(C.WEEKONLINEDRIVERCNT/A.DRIVERCNT,3)  ELSE 0.000 END) WEEKONLINEDRIVERRATE,
...
;"
/home/xiaoju/hadoop-1.0.4/bin/hadoop fs -touchz /user/xiaoju/data/bi/app/drivernatureregthreeweek/$V_PARYEAR/$V_PARMONTH/$V_PARDAY/_SUCCESS