...
QUERY="SELECT \
       ...
       passenger_id,\
       address_id,\
       REPLACE(REPLACE(REPLACE(invoice_title,'\r',''),'\n',''),'||','') AS invoice_title,\
       REPLACE(REPLACE(REPLACE(invoice_content,'\r',''),'\n',''),'||','') AS invoice_content,\
       invoice_value,\
       send_status,\
       DATE_FORMAT(_create_time,'%Y-%m-%d %H:%i:%s') AS _create_time,\
       DATE_FORMAT(_modify_time,'%Y-%m-%d %H:%i:%s') AS _modify_time,\
       _status \
       FROM g_invoice WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
