...
TABLE="withdraw_batch_part"
QUERY="SELECT \
billid,\
REPLACE(REPLACE(REPLACE(withdrawid,'\r',''),'\n',''),'||','') AS withdrawid,\
REPLACE(REPLACE(REPLACE(tenpay_drawid,'\r',''),'\n',''),'||','') AS tenpay_drawid,\
REPLACE(REPLACE(REPLACE(packageid,'\r',''),'\n',''),'||','') AS packageid,\
REPLACE(REPLACE(REPLACE(bank_card,'\r',''),'\n',''),'||','') AS bank_card,\
REPLACE(REPLACE(REPLACE(bank_type,'\r',''),'\n',''),'||','') AS bank_type,\
status,\
REPLACE(REPLACE(REPLACE(user_name,'\r',''),'\n',''),'||','') AS user_name,\
REPLACE(REPLACE(REPLACE(user_phone,'\r',''),'\n',''),'||','') AS user_phone,\
amount,\
bank_acc_type,\
REPLACE(REPLACE(REPLACE(bank_area,'\r',''),'\n',''),'||','') AS bank_area,\
REPLACE(REPLACE(REPLACE(bank_city,'\r',''),'\n',''),'||','') AS bank_city,\
REPLACE(REPLACE(REPLACE(subbank_name,'\r',''),'\n',''),'||','') AS subbank_name,\
REPLACE(REPLACE(REPLACE(details,'\r',''),'\n',''),'||','') AS details,\
REPLACE(REPLACE(REPLACE(retcode,'\r',''),'\n',''),'||','') AS retcode,\
REPLACE(REPLACE(REPLACE(retmsg,'\r',''),'\n',''),'||','') AS retmsg,\
DATE_FORMAT(notify_time,'%Y-%m-%d %H:%i:%s') AS notify_time,\
DATE_FORMAT(handling_time,'%Y-%m-%d %H:%i:%s') AS handling_time,\
DATE_FORMAT(done_time,'%Y-%m-%d %H:%i:%s') AS done_time,\
DATE_FORMAT(tobank_time,'%Y-%m-%d %H:%i:%s') AS tobank_time,\
DATE_FORMAT(ret_ticket_time,'%Y-%m-%d %H:%i:%s') AS ret_ticket_time,\
...
FROM withdraw_batch_part_${y}${m} WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
