...
QUERY="SELECT \
       ...
       company_id,\
       total_fee,\
       REPLACE(REPLACE(REPLACE(fee_content,'\r',''),'\n',''),'||','') AS fee_content,\
       didi_income,\
       driver_income,\
       third_party_income,\
       pay_status,\
       REPLACE(REPLACE(REPLACE(pay_result,'\r',''),'\n',''),'||','') AS pay_result,\
       close_status,\
       REPLACE(REPLACE(REPLACE(close_result,'\r',''),'\n',''),'||','') AS close_result,\
       junkid,\
       ...
       _status \
       FROM g_order_pay_010 WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
