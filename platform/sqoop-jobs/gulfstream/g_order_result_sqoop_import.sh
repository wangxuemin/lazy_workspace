...
QUERY="SELECT \
       ...
       tip_fee,\
       other_fee,\
       REPLACE(REPLACE(REPLACE(other_fee_desc,'\r',''),'\n',''),'||','') AS other_fee_desc,\
       pay_tip,\
       total_fee,\
       DATE_FORMAT(_create_time,'%Y-%m-%d %H:%i:%s') AS _create_time,\
       DATE_FORMAT(_modify_time,'%Y-%m-%d %H:%i:%s') AS _modify_time,\
       _status \
       FROM g_order_result_010 WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
