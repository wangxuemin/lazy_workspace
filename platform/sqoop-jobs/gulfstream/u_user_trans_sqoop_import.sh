...
bill_type,\
trans_type,\
REPLACE(REPLACE(REPLACE(action_type,'\r',''),'\n',''),'||','') AS action_type,\
cost,\
amount,\
prev_balance,\
balance,\
prev_f_balance,\
freeze_balance,\
REPLACE(REPLACE(REPLACE(partner_userid,'\r',''),'\n',''),'||','') AS partner_userid,\
partner_user_type,\
REPLACE(REPLACE(REPLACE(partner_user_name,'\r',''),'\n',''),'||','') AS partner_user_name,\
REPLACE(REPLACE(REPLACE(partner_user_phone,'\r',''),'\n',''),'||','') AS partner_user_phone,\
partner_accountid,\
...
for(( i=0;i<10;i++));do
    for(( dbi=0,j=1;dbi<100;dbi++,j++));do
        ...
        if [[ -z ${sql} ]]; then
            sql="${QUERY} FROM usertrans${dbii}.${TABLE}${i} WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59'"
        else
            sql="${sql} UNION ALL ${QUERY} FROM usertrans${dbii}.${TABLE}${i} WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59'"
        fi

        if [ $j -eq 25 ];then

            QUERY1="$sql AND \$CONDITIONS"
