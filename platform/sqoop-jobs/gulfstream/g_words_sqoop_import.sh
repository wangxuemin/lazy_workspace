...
QUERY="SELECT \
       id,\
       REPLACE(REPLACE(REPLACE(word,'\r',''),'\n',''),'||','') AS word,\
       REPLACE(REPLACE(REPLACE(pinyin,'\r',''),'\n',''),'||','') AS pinyin,\
       REPLACE(REPLACE(REPLACE(pinyin_s,'\r',''),'\n',''),'||','') AS pinyin_s,\
       REPLACE(REPLACE(REPLACE(pinyin_y,'\r',''),'\n',''),'||','') AS pinyin_y,\
       type,\
       REPLACE(REPLACE(REPLACE(unicode,'\r',''),'\n',''),'||','') AS unicode,\
       REPLACE(REPLACE(REPLACE(search_index,'\r',''),'\n',''),'||','') AS search_index,\
       DATE_FORMAT(_create_time,'%Y-%m-%d %H:%i:%s') AS _create_time,\
       DATE_FORMAT(_modify_time,'%Y-%m-%d %H:%i:%s') AS _modify_time,\
       _status \
       FROM g_words WHERE \$CONDITIONS"
