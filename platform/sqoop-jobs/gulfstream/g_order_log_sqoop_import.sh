...
QUERY="SELECT \
       ...
       REPLACE(REPLACE(REPLACE(code,'\r',''),'\n',''),'||','') AS code,\
       REPLACE(REPLACE(REPLACE(remark,'\r',''),'\n',''),'||','') AS remark,\
       REPLACE(REPLACE(REPLACE(operator,'\r',''),'\n',''),'||','') AS operator,\
       DATE_FORMAT(_create_time,'%Y-%m-%d %H:%i:%s') AS _create_time,\
       DATE_FORMAT(_modify_time,'%Y-%m-%d %H:%i:%s') AS _modify_time,\
       _status \
       FROM g_order_log_010 WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
