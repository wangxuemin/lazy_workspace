...
status,\
currency,\
DATE_FORMAT(rollback_time,'%Y-%m-%d %H:%i:%s') AS rollback_time,\
...
for(( i=0;i<10;i++));do
if [[ -z ${sql} ]]; then
    sql="${QUERY} FROM userbill${i}.${TABLE}_${y}${m}${d}"
else
    sql="${sql} UNION ALL ${QUERY} FROM userbill${i}.${TABLE}_${y}${m}${d}"
fi
done
