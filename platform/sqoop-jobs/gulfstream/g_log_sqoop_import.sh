...
QUERY="SELECT \
       log_id,\
       REPLACE(REPLACE(REPLACE(table_name,'\r',''),'\n',''),'||','') AS table_name,\
       table_record_id,\
       type,\
       REPLACE(REPLACE(REPLACE(ip,'\r',''),'\n',''),'||','') AS ip,\
       admin_id,\
       REPLACE(REPLACE(REPLACE(admin_name,'\r',''),'\n',''),'||','') AS admin_name,\
       content,\
       ...
       _status \
       FROM g_log WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
