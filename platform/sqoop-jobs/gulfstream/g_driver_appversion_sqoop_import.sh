...
TABLE="g_driver_appversion"
QUERY="SELECT \
       id,\
       driver_id,\
       app_type,\
       REPLACE(REPLACE(REPLACE(appversion,'\r',''),'\n',''),'||','') AS appversion,\
       REPLACE(REPLACE(REPLACE(sysversion,'\r',''),'\n',''),'||','') AS sysversion,\
       REPLACE(REPLACE(REPLACE(model,'\r',''),'\n',''),'||','') AS model,\
       REPLACE(REPLACE(REPLACE(imei,'\r',''),'\n',''),'||','') AS imei,\
       imei_status,\
       REPLACE(REPLACE(REPLACE(imsi,'\r',''),'\n',''),'||','') AS imsi,\
       up,\
       down,\
       down_map,\
       down_data,\
