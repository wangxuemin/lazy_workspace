...
TABLE="g_confirm_code"
QUERY="SELECT \
confirm_code_id,\
REPLACE(REPLACE(REPLACE(phone,'\r',''),'\n',''),'||','') AS phone,\
REPLACE(REPLACE(REPLACE(sendcode,'\r',''),'\n',''),'||','') AS sendcode,\
DATE_FORMAT(sendtime,'%Y-%m-%d %H:%i:%s') AS sendtime,\
REPLACE(REPLACE(REPLACE(token,'\r',''),'\n',''),'||','') AS token,\
DATE_FORMAT(_create_time,'%Y-%m-%d %H:%i:%s') AS _create_time,\
DATE_FORMAT(_modify_time,'%Y-%m-%d %H:%i:%s') AS _modify_time,\
_status \
FROM g_confirm_code WHERE _create_time BETWEEN '$v_executeDate 00:00:00' AND '$v_executeDate 23:59:59' AND \$CONDITIONS"
