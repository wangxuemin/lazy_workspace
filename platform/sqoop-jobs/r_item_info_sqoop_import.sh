...
sqoop import -D mapred.job.name='r_item_info_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --query "SELECT itemid,listid,replace(replace(replace(phone,'\r',''),'\n',''),'|','') as phone,DATE_FORMAT(create_time,'%Y-%m-%d %H:%i:%s') AS create_time,DATE_FORMAT(modify_time,'%Y-%m-%d %H:%i:%s') AS modify_time,DATE_FORMAT(audit_time,'%Y-%m-%d %H:%i:%s') AS audit_time,replace(replace(replace(openid,'\r',''),'\n',''),'|','') as openid,replace(replace(replace(nickname,'\r',''),'\n',''),'|','') as nickname,replace(replace(replace(head_url,'\r',''),'\n',''),'|','') as head_url, replace(replace(replace(comment,'\r',''),'\n',''),'|','') as comment,amount,status,comment_status FROM r_item_info WHERE ( (create_time BETWEEN '${v_executeDate} 00:00:00' AND '${v_executeDate} 23:59:59') or (modify_time BETWEEN '${v_executeDate} 00:00:00' and '${v_executeDate} 23:59:59' ) ) and \$CONDITIONS " --delete-target-dir --split-by itemid -m 5 --fields-terminated-by '\001' --null-string '' --null-non-string '' --as-textfile --target-dir /user/xiaoju/data/bi/mysql/r_item_info/${year}/${month}/${day}/
