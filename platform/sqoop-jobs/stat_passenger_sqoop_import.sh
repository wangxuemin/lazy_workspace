...
sqoop import -D mapred.job.name='stat_passenger_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull&tinyInt1isBit=false" --username ${db_username} --password ${db_password} --query "SELECT id,pid,model,replace(replace(imei,'\r',''),'\n','') as imei,token,app_version,platform FROM stat_passenger WHERE \$CONDITIONS" --split-by id -m 3 --fields-terminated-by '\001'  --null-string ''  --null-non-string '' --as-textfile --delete-target-dir  --target-dir "/user/xiaoju/data/bi/mysql/stat_passenger/$v_year/$v_month/$v_day/"
