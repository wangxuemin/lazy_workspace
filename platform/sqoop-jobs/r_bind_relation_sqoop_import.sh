...
#sqoop import -D mapred.job.name='r_bind_relation_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --query "SELECT bindid,replace(replace(replace(phone,'\r',''),'\n',''),'|','') as phone,DATE_FORMAT(create_time,'%Y-%m-%d %H:%i:%s') AS create_time, DATE_FORMAT(modify_time,'%Y-%m-%d %H:%i:%s') AS modify_time FROM r_bind_relation WHERE create_time BETWEEN '${v_executeDate} 00:00:00' AND '${v_executeDate} 23:59:59' and \$CONDITIONS " --delete-target-dir --split-by bindid -m 5 --fields-terminated-by '\001' --null-string '' --null-non-string '' --as-textfile --target-dir /user/xiaoju/data/bi/mysql/r_bind_relation/${year}/${month}/${day}/
sqoop import -D mapred.job.name='r_bind_relation_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --query "SELECT bindid,replace(replace(replace(phone,'\r',''),'\n',''),'|','') as phone,DATE_FORMAT(create_time,'%Y-%m-%d %H:%i:%s') AS create_time, DATE_FORMAT(modify_time,'%Y-%m-%d %H:%i:%s') AS modify_time FROM r_bind_relation WHERE \$CONDITIONS " --delete-target-dir --split-by bindid -m 5 --fields-terminated-by '\001' --null-string '' --null-non-string '' --as-textfile --target-dir /user/xiaoju/data/bi/mysql/r_bind_relation/${year}/${month}/${day}/
