...
/home/xiaoju/sqoop-1.4.4.bin__hadoop-0.23/bin/sqoop import -D mapred.job.name='city_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --table city_info -m 1 --fields-terminated-by '\001' --null-string '' --null-non-string '' --as-textfile --delete-target-dir --target-dir "/user/xiaoju/data/bi/mysql/city/$v_year/$v_month/$v_day/"
