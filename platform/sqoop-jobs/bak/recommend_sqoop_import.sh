...
sqoop import -D mapred.job.name="${tablename}_sqoop_import" --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --query "select id,recommend_id,type,phone,createtime,usetime,is_used,area,audittime,regtime,remark from recommend where createtime ='${v_executeDate}' and \$CONDITIONS " --delete-target-dir --split-by id -m 5 --fields-terminated-by '\001'  --null-string ''  --null-non-string '' --as-textfile --target-dir /user/xiaoju/data/bi/mysql/${tablename}/${year}/${month}/${day}/
