...
sqoop import -D mapred.job.name='driver_account_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull&tinyInt1isBit=false" --username ${db_username} --password ${db_password} --table driver_account  -m 1 --fields-terminated-by '\001'  --null-string ''  --null-non-string '' --as-textfile --delete-target-dir  --target-dir "/user/xiaoju/data/bi/mysql/weixin/driver_account/$v_year/$v_month/$v_day/"
