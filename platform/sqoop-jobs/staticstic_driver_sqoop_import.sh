...
sqoop import -D mapred.job.name='op_staticstic_driver_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}?zeroDateTimeBehavior=convertToNull" --username ${db_username} --password ${db_password} --query "select id,dphone,DATE_FORMAT(date,'%Y-%m-%d'),area,onlineTime,sumOrder,recommend_d,recommend_p,did from op_staticstic_driver where date ='${v_executeDate}' and \$CONDITIONS " --delete-target-dir --split-by id -m 1 --fields-terminated-by '\001'  --null-string ''  --null-non-string '' --as-textfile --target-dir /user/xiaoju/data/bi/mysql/op_staticstic_driver/${year}/${month}/${day}/
