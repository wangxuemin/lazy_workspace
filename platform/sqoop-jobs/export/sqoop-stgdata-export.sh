...
if [ -z "$v_paramDate" ];then
	v_executeDate=`date --date="$v_currentDate-1 month" +%Y-%m-%d`
else
...
#    cat etc/sqoop-export-table.conf | while read line; do
    for line in `cat etc/sqoop-export-stgdata-table.conf`; do
        table=${line%%:*}
        column=${line##*:}

        lowertablename=`echo ${table}|awk '{ print tolower( $1 );}'`
        hdfspath=${hive_app_path}/${lowertablename}/${year}/${month}
...
sqoop_one_table()
{
    for line in `egrep -i "^$1" etc/sqoop-export-stgdata-table.conf`; do
        table=${line%%:*}
        column=${line##*:}
...
title="SQOOP stgdata export occur Exception"
carbon=lihan@diditaxi.com.cn
reciver=lihan@diditaxi.com.cn
#reciver=bigdata-p@diditaxi.com.cn

if [ -z ${_table} ];then
    sqoop_all_table > log/sqoop_stgdata_table.${v_executeDate}.log 2>&1
    egrep "Cannot find|Caused.*" log/sqoop_stgdata_table.${v_executeDate}.log > log/${v_executeDate}.failed_sqoop_stgdata_export.lst

    if [ -s log/${v_executeDate}.failed_sqoop_stgdata_export.lst ];then
        cat log/${v_executeDate}.failed_sqoop_stgdata_export.lst | mutt -s "${title}" -c "${carbon}" "${reciver}"
    fi
else
