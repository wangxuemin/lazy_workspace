...
if [ ! -e stgdata_create_table.sql ];then
    echo Conot find file mart_create_table.sql
    exit
fi
day=`date +%Y-%m-%d`
mkdir -p etc
mv etc/sqoop-export-stgdata-table.conf etc/sqoop-export-stgdata-table.${day}.conf

egrep -v "USE|DROP|KEY|ENGINE|^/|^--" stgdata_create_table.sql |tr -s '\n' | awk 'BEGIN{
...
}' | egrep -v "DataMartIndex|BIOrderBonus|finance_report|Admin|admins_menu|city_info|investor_wechat_month" | tee etc/sqoop-export-stgdata-table.conf
