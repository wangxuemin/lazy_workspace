...
sqoop import -D mapred.job.name='order_comment_sqoop_import' --connect "jdbc:mysql://${masterdb_ip}:${masterdb_port}/${app_dbname}" --username ${db_username} --password ${db_password} --query "SELECT cmtid,oid,ordertype,pphone,dphone, replace(replace(pcontent,'\r',''),'\n','') as pcontent,pcommenttype,DATE_FORMAT(pctime,'%Y-%m-%d') AS pctime,replace(replace(pcomplaint,'\n',''),'\r','') as pcomplaint,pcomplainttype,DATE_FORMAT(ptime,'%Y-%m-%d') AS ptime,pstatus,dcommenttype, replace(replace(dcontent,'\r',''),'\n','') as dcontent,replace(replace(dcomplaint,'\n',''),'\r','') as dcomplaint,dcomplainttype,DATE_FORMAT(dtime,'%Y-%m-%d') AS dtime,dstatus,credibility,neatness,punctual,replace(replace(remark,'\r',''),'\n','')  as remark,replace(replace(opname,'\n',''),'\r','') as opname,channel,AREA,DATE_FORMAT(createtime,'%Y-%m-%d') AS createtime FROM order_comment where createtime between '0000-00-00 00:00:00' and '${v_executeDate} 23:59:59' and \$CONDITIONS " --split-by cmtid -m 1 --fields-terminated-by '\001' --null-string '' --null-non-string '' --as-textfile --delete-target-dir --target-dir /user/xiaoju/data/bi/mysql/order_comment/${year}/${month}/${day}/
